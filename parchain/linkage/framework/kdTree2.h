...
template<int dim, class objTT, class nodeInfo>
class kdNode {
  public:
  ...
  static const intT leafSize = 4;//16
  ...
  inline bool itemInBox(pointT pMin1, pointT pMax1, objT* item) {
    ...
    inline double pointBoxDistance(point<dim> p, pointT pMin, pointT pMax) {
        for (int d = 0; d < dim; ++ d) {
  ...
  //return exclude or overlap
  inline int boxBallCompare(point<dim> center, double r, pointT pMin, pointT pMax) {
  ...
  // inline int ballCompare(point<dim> center, double r, pointT pMin, pointT pMax) {
  //   point<dim> boxcenter = pMin.p;
  //   for (int i=0; i<dim; ++i) {
  //     boxcenter.updateX(i, (pMin[i] + pMax[i])/2);}

  //   double dist = boxcenter.pointDist(center);
  //   if (dist>r) return boxExclude;


  //   if (dist < r) return boxInclude;
  //   else return boxOverlap;
  // }

  inline bool itemInBall(point<dim> center, double r, objT* item) {
  ...
  void knnRangeHelper(objT* q, pointT qMin, pointT qMax, double radius, kbufT *out);
  void knnRange(objT* q, double radius, kbufT *out);
  ...
  inline bool isLeaf() {return n<=leafSize;}
  inline objT *operator[](int i) {return items[i];}
  ...
  inline floatT Diameter() {
    if (diameter < 0) {
      diameter = 0;
      for (intT d = 0; d < dim; ++ d) {
        diameter += (pMax[d] - pMin[d]) * (pMax[d] - pMin[d]);
      }
...
public:
  ...
  inline void kdTreeHelper(intT n,  bool parallel, bool alloc_nodes){
    if(n == 0){
  ...
  // rebuild kdtree using allocated memory
  inline void kdTreeRebuild(objT* P, intT n, bool parallel = true){
    parallel_for (intT i=0; i<n; ++i) items[i]=&P[i];
    kdTreeHelper(n,  parallel, /*alloc_nodes = */ false);
  }
...
#endif
    parallel_for (intT i=0; i<n; ++i) items[i]=&P[i];
    kdTreeHelper(n, parallel, /*alloc_nodes = */ true);
  }
