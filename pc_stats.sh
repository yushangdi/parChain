...
[ -d "outputs/stats_outputs_${METHOD}${SUFFIX}" ] || mkdir "outputs/stats_outputs_${METHOD}${SUFFIX}"
...
for wk in "${workers[@]}"; do
	...
	if [[ "${wk}" -eq 1 ]];then
            echo "CILK_NWORKERS=${wk} ./parchain/linkage/framework/linkage -method $METHOD -r 1 -d ${dims[$ind]} -cachesize ${CACHESIZE} -eps ${eps[$ind]} ./datasets/${dataset}.pbbs > outputs/stats_outputs_${METHOD}${SUFFIX}/stats_${METHOD}${SUFFIX}_${dataset}_${wk}th.txt"
            CILK_NWORKERS=${wk} ./parchain/linkage/framework/linkage -method $METHOD -r 1 -d ${dims[$ind]} -cachesize ${CACHESIZE} -eps ${eps[$ind]} ./datasets/${dataset}.pbbs \
                > outputs/stats_outputs_${METHOD}${SUFFIX}/stats_${METHOD}${SUFFIX}_${dataset}_${wk}th.txt
        else
            echo "CILK_NWORKERS=${wk} numactl -i all ./parchain/linkage/framework/linkage -method $METHOD -r 1 -cachesize ${CACHESIZE} -d ${dims[$ind]} -eps ${eps[$ind]} ./datasets/${dataset}.pbbs > outputs/stats_outputs_${METHOD}${SUFFIX}/stats_${METHOD}${SUFFIX}_${dataset}_${wk}th.txt"
